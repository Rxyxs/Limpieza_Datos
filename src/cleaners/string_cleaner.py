"""Limpieza de texto: espacios, parseo de monedas, unificación fuzzy de nombres."""
...
CURRENCY_TOKENS = ["US$", "USD", "EUR", "GBP", "$", "€", "£"]
...
def parse_currency(value) -> float | None:
    """Convierte un string de moneda (símbolos y separadores mezclados) a float.

    Soporta formato US ("$1,200.50", coma de miles / punto decimal) y formato europeo
    ("1200,50 €", punto de miles / coma decimal), detectando cuál aplica por la
    posición relativa de la última coma y el último punto.
    """
...
def unify_similar_names(series: pd.Series, threshold: float = 80) -> pd.Series:
    ...
    grupo se conserva como canónico el primer nombre encontrado al recorrer de más a
    menos frecuente. Los valores nulos o vacíos se preservan como nulos.
    """
