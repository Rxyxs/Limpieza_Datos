"""Aplana la columna JSON anidada `user_metadata` en columnas tabulares."""
...
def normalize_json_column(df: pd.DataFrame, column: str = "user_metadata", prefix: str | None = None) -> pd.DataFrame:
    """Reemplaza `column` (JSON anidado de profundidad variable) por columnas planas.

    Cada ruta anidada se convierte en una columna `<prefix>_<ruta>` (p. ej.
    `user_metadata_browser_name`); las listas se serializan como texto separado por
    comas para conservar una fila por ticket.
    """
