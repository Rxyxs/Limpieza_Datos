...
Leer las etiquetas numéricas impresas en vez de solo la altura de las barras importa acá: el RMSE de los tres modelos coincide hasta el tercer decimal (0,00527-0,00529), y el R² de los tres queda dentro de 0,006 de cero en cualquier dirección -- diferencias visualmente invisibles a esta escala de gráfico, que es justamente el motivo de etiquetar cada barra con su número exacto en vez de dejar que el lector calcule a ojo unos pocos píxeles de diferencia de altura.
...
El orden es consistente en las tres métricas (XGBoost primero, MLP segundo, baseline estacional último) -- vale la pena notarlo porque sería una señal de alerta si, digamos, un modelo ganara en R² pero perdiera en RMSE, ya que para un mismo test set held-out ambas métricas están matemáticamente relacionadas y no deberían discrepar sobre qué modelo queda más cerca de los valores reales en promedio.
...
A diferencia de la versión de este mismo gráfico en el dominio minero (donde la dispersión de residuales visiblemente se ensancha en valores predichos más altos), acá la dispersión de residuales se mantiene prácticamente plana en todo el rango de rendimiento predicho -- medido directamente, la desviación estándar de los residuales en la mitad baja de las predicciones (367 kg/ha) y la mitad alta (371 kg/ha) apenas difieren, lo que significa que la precisión real de este modelo es genuinamente uniforme a través del rango de rendimiento, no solo uniforme a simple vista.
...
Las barras de la MLP y de XGBoost quedan lo bastante cerca (0,872 vs. 0,884 R²) como para verse casi idénticas a esta escala de gráfico, que es la impresión honesta correcta -- la historia real de este dominio es "ambos enfoques reales de modelamiento funcionan bien y superan al baseline por un margen amplio", no "una arquitectura exótica le gana decisivamente a otra", y el gráfico está dibujado con la suficiente sobriedad como para no sobrevender una diferencia que es pequeña en relación a la ventaja que ambos modelos comparten sobre el baseline.
...
XGBoost también le gana a la MLP en cada métrica acá (0,938 vs. 0,922 R²), el mismo orden visto en los gráficos de comparación de los dominios minero y agrícola -- XGBoost es el mejor modelo en tres de los cuatro dominios de este proyecto, con la única excepción del dominio financiero, donde el R²=0,0040 nominal de la MLP le gana por poco al -0,0021 de XGBoost, una diferencia lo bastante chica (ambos cerca de cero) como para no contradecir realmente el patrón más amplio.
